...
class RoarCompetitionRule:
    ...
    async def respawn(
        ...
        self._last_vehicle_location = self.vehicle.get_3d_location()
        self.furthest_waypoints_index = 0
def run_update_py(kp_value):
      subprocess.run(["python", "update_kp.py","C:\\Users\phana\\Monza\\ROAR_Competition\\competition_code\\submission.py", str(kp_value)])

def get_current_kp():
    with open("C:\\Users\phana\\Monza\\ROAR_Competition\\competition_code\\submission.py", "r") as file:
...
async def main_loop():
    while True:
        ...
        RoarCompetitionSolution = submission.RoarCompetitionSolution
        ...
        current_kp = get_current_kp()
        if evaluation_result is not None:
            print("Solution finished in {} seconds".format(evaluation_result["elapsed_time"]))
            break
        else:
            print("Solution failed to finish in time. Kp: " + str(current_kp))
            run_update_py(current_kp - .5)
